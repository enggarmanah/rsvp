...
public class OAuthUtil {
	...
	private static String[] authProviders = { "Facebook", "Google", "Twitter", "Yahoo!", "Flickr", "ImaGur", "LinkedIn",
			"Windows Live", "Instagram", "github", "Vimeo", "Evernote", "tumblr.", "foursquare", };
	...
	public static void redirect(String url) {
		...
		int left = Window.getClientWidth()/2 - width/2 - 5;
		
		Window.open("../oAuthLogin", "_blank", "width="+ width +", height="+ height +", top="+ top +", left="+ left +", menubar=no, toolbar=no");
	}
	...
	public static boolean redirected() {
		...
		if (Location.getParameter("code") != null) {// facebook,google,github,windows
			return true;
		}
	...
	public static void updateLoginStatus()
    {
        ...
        if (OAuthUtil.alreadyLoggedIn())
        {
            GWT.log("Already logged in..showing Logout anchor");
            //showLogoutAnchor();
        }
        else
        {
            GWT.log("Showing Login anchor..");
            //showLoginAchor();
        }
        //updateLoginLabel();
    }

	public static Credential getCredential() throws OAuthException {
		String authProvider = getAuthProviderFromCookie();
		if (authProvider == null)
			return null;
		int ap = DEFAULT;
		...
		try {
			...
			throw new OAuthException("Could not convert authProvider " + authProvider + " to Integer");
		}
		...
		default: {
			throw new OAuthException("ClientUtils.getCredential: Auth Provider " + authProvider + " Not implemented yet");
		}
		}
	}
	
	public static Credential getSvrCredential(Integer authProvider, String state, String code, String oauth_verifier) throws OAuthException {
		...
		case GITHUB: {
			Credential credential = new Credential();
			credential.setAuthProvider(ap);
			credential.setState(state);
			credential.setVerifier(code);
			return credential;
		}
		case GOOGLE:
		case WINDOWS_LIVE:
		...
		case TUMBLR: {
			Credential credential = new Credential();
			credential.setAuthProvider(ap);
			credential.setVerifier(oauth_verifier);
			return credential;
		}
	...
	public static void handleException(Throwable caught) {
		if (caught.getMessage().equals(OAuthUtil.SESSION_EXPIRED_MESSAGE)) {
			showSessionExpires();
		} else {
