...
public class AdminStreetListView extends BaseView implements IAdminStreetListView {
	
	interface ModuleUiBinder extends UiBinder<Widget, AdminStreetListView> {}
	...
	List<GenericBean<StreetBean>> list;
	...
	public void show() {
		...
		dialogBox.setText("Referensi Jalan");
		dialogBox.center();
		dialogBox.setPopupPosition(dialogBox.getPopupLeft(), Constant.POPUP_L1_TOP);
		dialogBox.show();
		
		this.list = new ArrayList<GenericBean<StreetBean>>();
		this.size = list.size();
	...
	public void setList(List<GenericBean<StreetBean>> list) {
	...
	public void loadContent() {
		...
		List<GenericBean<StreetBean>> sublist = list.subList(fromIndex, toIndex);
		
		int i = fromIndex + 1;
		
		for (GenericBean<StreetBean> genericBean : sublist) {
			
			StreetBean streetBean = genericBean.getBean();
			HandlerManager handlerMgr = genericBean.getHandlerMgr();
			
			Label noLb = new Label(i + ".");
			Label nameLb = new Label(streetBean.getName());
			Label cityLb = new Label(streetBean.getCity().getName());
	...
	public void remove(GenericBean<StreetBean> genericBean) {
